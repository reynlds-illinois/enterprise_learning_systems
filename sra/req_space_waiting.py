import cx_Oracle, csv, sys, argparse, datetime
...
if statusCode == 'a':
    ...
    spacesWaitingSql = f"""SELECT SR.SPACE_ID SPACE_ID,
        S.CODE CODE,
        S.STATUS_ADMIN_DESC STATUS,
        CM.CREATE_METHOD_DESC METHOD,
        to_char(SR.REQUEST_DATE,'YYYYMMDDHH24MISS') || SR.SPACE_ID SORT_KEY,
...
if len(results) > 0:
    ...
    if nextAction == 'a':
        ...
        with open(targetFilePath, 'w') as exportFile:
            ...
            print('')
        try:
            #r = upload2Box(boxClient, boxTargetFolderId, targetFilePath, targetFileName)
