...
cd2ErrorFile=$cd2LogLocation$now"_"$CD2_ACTION"_error.log"
...
if [ -s "$cd2ErrorFile" ]; then
    subjectLine=$today"_CD2_"$CD2_ACTION"_ERRORS"
    cat $cd2ErrorFile | mail -s $subjectLine NOTIFYM3@illinois.edu
else
    subjectLine=$today"_CD2_"$CD2_ACTION"_PASS"
    echo $subjectLine | mail -s $subjectLine NOTIFYM3@illinois.edu
fi
