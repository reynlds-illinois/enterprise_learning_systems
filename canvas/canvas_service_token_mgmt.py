...
serviceAccts = ['84','94','109','7016','8994','125511','129156','132703','319151','319349','336978','355105','360354','377818','378454','381230','400969','417093','420084','426543', '458951', '465258']
...
boxParentFolderID = '342315882395'
...
def canvasListUserTokens(canvasUserTokensCSV, canvasAllUsers):
    ...
    with open(canvasUserTokensCSV, 'r') as file:
        ...
        for line in allTokens:
            if line[5] == '170000000000016' and line[0] in serviceAccts:
                canvasUserID = line[0]
                canvasUser = line[1]
                tokenHint = line[2]
...
while True:
    ...
    while actionChoice != 'l' and actionChoice != 'n' and actionChoice != 'd':
        print('Choose an action: (l)ist all user tokens')
        print('                  (n)ew service acct token')
    ...
    if actionChoice == 'l':
        ...
        yesNo = ''
        FY = input('Enter the fiscal year for this token: ')
        print()
        netID = input('Enter the NetID of the SERVICE USER ACCT that will receive the token:  ')
        print()
        requestorNetID = input("Enter the NetID of the RESPONSIBLE PARTY for the service account:  ")
        print()
        ...
        requestorUserInfo = canvasGetUserInfo(requestorNetID)
        canvasUserID = canvasUserInfo[0]
        displayName = canvasUserInfo[3]
        print()
        #reason = input('Enter a short reason for this request: ')
        reason = f"Annual rotation of service account token on {today}"
        print()
        #expiryDate = generateExpiryDate()
        expiryDate = input("Enter the new expiration date in this format YYYY-MM-DD:  ")
        print()
        yesNo = ''
        while yesNo != 'y' and yesNo != 'n':
            print('  ===== Summary of Changes =====')
            print(f'  = Service Acct NetID:  {netID}')
            print(f'  = Responsible User:    {requestorNetID}')
            print(f'  = Fiscal Year:         {FY}')
            print(f'  = Expiry Date:         {expiryDate}')
            print(f'  = Reason:              {reason}')
            print('  ==============================')
            print()
            yesNo = input('>>> Continue creating new token (y/n)? ').lower().split()[0]
            print()
        if yesNo == 'y':
            try:
                ...
                newTokenCreated = False
                print()
        if newTokenCreated == True:
            print()
            yesNo = ''
            while yesNo != 'y' and yesNo != 'n':
                yesNo = input('>>> Continue upload to BOX (y/n)? ').lower().split()[0]
                print()
            if yesNo == 'y':
                tokenTempFileName = f'{netID}_{FY}_token.txt'
    ...
    ''')
                    ...
                    print()
                    print("|----------------------------------------------------------------------")
                    print("| A service account token for the Canvas API has been generated for your use.")
                    print("| You can access the token in U of I BOX at this location:")
                    print('|')
                    print(f'| {boxSharedLink}')
                    print('|')
                    print('| Please update your application with the new token at your earliest convenience.')
                    print("|----------------------------------------------------------------------")
                    print()
    ...
    #
        ...
        #        break
    answer = input("Continue with another SERVICE ACCT token (y/n)? ").strip().lower()
    if answer != 'y':
        break
print()
print(f'Exiting {canvasApi} service acct token management...')
print()
