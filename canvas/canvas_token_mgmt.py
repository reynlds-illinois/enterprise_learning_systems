...
def generateExpiryDate():
    ...
    validTokenTypes = ['s', 'u']
    tokenType = ''
    while tokenType not in validTokenTypes:
        tokenType = input('  >>> Token type:  (u)ser or (s)ervice account? ').lower().split()[0]
        print()
    if tokenType == 's':
        while True:
            try:
                daTe = input('Enter an expiration date in this format YYYY-MM-DD:  ')
                newExpiryDate = datetime.strptime(daTe, '%Y-%m-%d')
                break
            except:
...
''')
                ...
                print()
                print()
                print('  > NOTE:  A newly generated token will not show up in the token listing until')
                print('  >        the scheduled canvas_user_token_report has been run. It can also be')
                print('  >        manually generated by running the following command:')
                print('  >')
                print('  >        /var/lib/canvas-mgmt/bin/canvas_user_token_report.sh')
                print()
